...
export const TrainingSummary = ({ days }: { days: number | null }) => {
    ...
    const cards = [
        { label: 'セッション', value: summary?.sessions ?? 0, suffix: '回', icon: CalendarDays },
        { label: '総負荷', value: Math.round(summary?.totalLoad ?? 0).toLocaleString(), suffix: '', icon: Activity },
        { label: '最高重量', value: summary?.maxWeight ?? 0, suffix: 'kg', icon: Trophy },
        { label: 'ストリーク', value: summary?.streak ?? 0, suffix: '日', icon: Flame },
    ];
